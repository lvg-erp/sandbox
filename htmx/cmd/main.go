...
func main() {
	fmt.Println("Starting monitor server on port 8097")
	s := NewServer()
	go func(srv *server) {
		for {
	...
	<div hx-swap-oob="innerHTML:#update-timestamp">
		<p><i style="color: green" class="fs fs-circle"></i>` + timeStamp + `</p>
	</div>
	...
	err := http.ListenAndServe(":8097", &s.mux)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
