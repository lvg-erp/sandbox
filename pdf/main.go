...
func main() {
	...
	err = doc.Save("output/invoice.pdf")
	if err != nil {
		log.Fatal(err.Error())
	}
...
func addHeader(m core.Maroto) {
	...
	m.AddRow(20, text.NewCol(12, "CODEHEIM",
		props.Text{
			Top:   5,
...
func addInvoiceDetails(m core.Maroto) {
	m.AddRow(10,
		text.NewCol(6, "Date: "+time.Now().Format("02 Jan 2006"),
			props.Text{
				Align: align.Left,
				Size:  10,
			},
...
func addFooter(m core.Maroto) {
	...
	m.AddRow(40, signature.NewCol(6, "Authorized Signature", props.Signature{FontFamily: fontfamily.Courier}),
